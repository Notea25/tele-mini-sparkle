...
const FormationField = ({
  ...
  useEffect(() => {
    const updateCardSize = () => {
      ...
      if (width < 375) {
        ...
        const minWidth = 62; // Изменено с 64px на 62px
        const maxWidth = 70; // На 375px чуть больше
        ...
        const minWidth = 70; // На 375px
        const maxWidth = 192; // На 768px
        const scale = (width - 375) / (768 - 375);
        cardWidth = minWidth + (maxWidth - minWidth) * scale;
      } else if (width < 1024) {
        // От 768px до 1024px - плавное масштабирование
        const minWidth = 192; // На 768px
        const maxWidth = 256; // На 1024px (в 4 раза больше мобильного)
        const scale = (width - 768) / (1024 - 768);
        cardWidth = minWidth + (maxWidth - minWidth) * scale;
      } else {
        // От 1024px и выше - плавное масштабирование до 1920px
        const minWidth = 256; // На 1024px
        const maxWidth = 320; // На 1920px (в 5 раз больше мобильного)
        const maxScreen = 1920;
        const scale = Math.min(1, (width - 1024) / (maxScreen - 1024));
        cardWidth = minWidth + (maxWidth - minWidth) * scale;
      }
      ...
      // Ограничиваем минимальный и максимальный размер
      cardWidth = Math.max(62, Math.min(320, cardWidth)); // Минимум 62px

      // Высота пропорциональна ширине (соотношение 82:64 = 1.28125, но для 62px будет ~79px)
      // Старое соотношение: 64px × 82px = 1.28125
      // Новое: 62px × 79.4px = 1.28125
  ...
  // Базовые значения для мобильного (320px)
  const mobileBaseWidth = 320;
  const mobilePadding = 3; // Немного уменьшили паддинги для мобильных
  const mobileGapFor2 = 14; // для 2 карточек (уменьшили)
  const mobileGapFor5 = 7; // для 5 карточек (уменьшили)
  ...
  // Вертикальные отступы между строками
  const rowSpacing = cardSize.height * 0.45; // 45% от высоты карточки (немного уменьшили)
    ...
    <div className="relative w-full">
      ...
      >
        ...
        <div
          className="absolute left-0 right-0 flex justify-center"
          style={{
            top: `calc(3% + ${cardSize.height}px + ${rowSpacing}px)`,
            gap: `${getRowGap(5)}px`,
          }}
        >
          {rows[2].map((slot, idx) => {
        ...
        <div
          className="absolute left-0 right-0 flex justify-center"
          style={{
            top: `calc(3% + ${cardSize.height * 2}px + ${rowSpacing * 2}px)`,
            gap: `${getRowGap(5)}px`,
          }}
        >
          {rows[3].map((slot, idx) => {
        ...
        <div
          className="absolute left-0 right-0 flex justify-center"
          style={{
            top: `calc(3% + ${cardSize.height * 3}px + ${rowSpacing * 3}px)`,
            gap: `${getRowGap(3)}px`,
          }}
        >
          {rows[4].map((slot, idx) => {
